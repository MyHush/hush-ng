      ...
      <div class="right-side">
        <div class="doc">
          ...
          <div v-if="installDone === true">
            <button class="button" style="font-weight: 600;">Launch HushNG</button><br><br>
          </div>
          <div v-else>
            <button class="button-info">Cancel setup</button><br><br>
          </div>
        </div>
        <div class="doc">
          <div class="title alt">Get Involved</div>
          <button class="alt" @click="open('https://github.com/MyHush')">Github</button>
          <button class="alt" @click="open('https://discord.gg/VfaZjyR')">Discord</button>
        </div>
      </div>
    </main>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      startInstall () {
        ...
        cmd.get(
            'mkdir -p ' +  require('os').homedir() + '/hush-ng/bin',
        ...
        if (platform == "linux") {
          ...
          for (var i = 0; i < self.downloadsWindows.length; i++) {
            self.download(self.downloadsWindows[i].url, path + '/' + self.downloadsWindows[i].component, platform, i)
          }
        }
        ...
        if (platform == "linux") {
          ...
          stream.once('open', function(fd) {
            ...
            stream.write("daemon=1\nserver=1\nrpcallowip=127.0.0.1\nrpcuser=rpcuser\nrpcpassword=" + Math.random().toString(36).slice(2) + "\n\nshowmetrics=1")
            stream.end();
          });
          self.installSteps[2].success = true
        } else if (platform == "win32") {
          var path = process.env.APPDATA + '/Roaming/Hush/'
          stream.once('open', function(fd) {
            stream.write("# HUSH Configuration File\n\n")
            stream.write("# This file has been automatically generated by Hush Config Generator. It may be further customized by hand only.\n\n")
            stream.write("# Creation date: " + compileTime + "\n\n")
